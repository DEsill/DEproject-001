...
def get_conversion_rate(conversion_rate_path):
    ...
    dt.to_csv(conversion_rate_path, index=False)
...
def merge_data(transaction_path, conversion_rate_path, output_path):
    ...
    final_df = transaction.merge(conversion_rate, how="left", righ_on="date", left_on="date")

    final_df["Price"] = final_df.apply(lambda x: x["Price"].replace("$",""), axis=1)
    final_df["Price"] = final_df["Price"].astype(float)

    final_df["THBPirice"] = final_df["Price"] * final_df["conversion_rate"]
